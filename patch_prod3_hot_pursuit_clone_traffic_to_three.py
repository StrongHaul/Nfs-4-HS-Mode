...
def cave() -> bytes:
    items: list[int | str | tuple[str, str, str, str]] = [
        ...
        # Cloning them there can corrupt the HP composition in PROD3 with the
        # two-racer frontend patch, turning the added slots into HSV racers.
        # Tournament traffic is initialized separately by FRONT and can safely
        # be topped up when the tournament provides only 1-2 traffic entries.
        addiu("t6", "zero", 2),
        bne("v1", "t6", "done"),
        nop(),
        slti("t6", "t1", MAX_CARS),
        ...
        "tournament_traffic_copy",
        lw("t7", 0x0000, "t8"),
        addiu("t8", "t8", 4),
        sw("t7", 0x0000, "t9"),
        addiu("t9", "t9", 4),
        addiu("t6", "t6", -1),
        bne("t6", "zero", "tournament_traffic_copy"),
        nop(),

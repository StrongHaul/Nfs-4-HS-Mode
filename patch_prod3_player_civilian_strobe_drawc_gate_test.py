...
BACKUP_SUFFIX = ".orig_before_prod3_player_civilian_strobe_drawc_gate_test"
RUNTIME_BASE = 0x8000F800

# DrawC_NightHeadlight:
#   if (R3DCar_InMenu != 0 || (carObj->speechInfo.speechSource & 2) != 0)
# The previous latch patch gives player civilian cars a palCopy flash phase,
# but this gate still skips the head/tail flash tables.  This hook lets only
# player-1 civilian cars enter the existing flash block.
HOOK_OFF = 0xB0B34
...
def cave() -> bytes:
    items: list[int | str | tuple[str, str, str, str]] = [
        # Original branch delay instruction: needed before either path reaches
        ...
        nop(),
        # Only regular civilian cars; leave player cop cars fully stock.
        lhu("t1", 0x08BC, "s2"),
        slti("t1", "t1", 0x16),
...
def main() -> int:
    ...
    print("player civilian DrawC strobe gate:", "reverted" if args.revert else "patched")
    return 0

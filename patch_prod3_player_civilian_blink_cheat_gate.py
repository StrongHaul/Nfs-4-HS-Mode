...
# off unless DuckStation writes 0001 here.
BLINK_CHEAT_ADDR = 0x800551F8
...
def make_gate(base_addr: int, *, disabled_addr: int, return_addr: int) -> bytes:
    items: list[int | str | tuple[str, str, str, str]] = [
        lui("t0", hi(BLINK_CHEAT_ADDR)),
        lhu("t1", lo(BLINK_CHEAT_ADDR), "t0"),
        addiu("t1", "t1", -1),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PROD3: gate ordinary-player blinking light feature behind cheat 800551F8."
    )
    ...
    caves = [
        (STROBE_GATE_OFF, STROBE_GATE_LEN, strobe_gate, "strobe gate"),
        (OBJECT_GATE_OFF, OBJECT_GATE_LEN, object_gate, "object gate"),
        (DRAW_GATE_OFF, DRAW_GATE_LEN, draw_gate, "draw gate"),
    ]
    for off, size, blob, name in caves:
        current = bytes(data[off : off + size])
        if current not in {bytes(size), blob}:
    ...
    print("enable:  800551F8 0001")
    print("disable: 800551F8 0000")
    return 0

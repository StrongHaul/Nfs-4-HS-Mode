...
git       30b7145 Add civilian player siren sound cheat
```

Проверено пользователем после добавления code-immediate gate для моргания обычных машин, условного `0/1`-переключателя visual-only damage и сирены на обычной машине игрока: зависаний не наблюдается, фары/поворотники/белые задние огни работают, полицейские спичи через громкоговоритель работают, damage-only-visual чит работает, звук сирены на неполицейской машине игрока работает.
...
- Кастомное моргание световых элементов обычных машин по умолчанию выключено и включается отдельным читом.
- Damage-only-visual по умолчанию выключен и включается одним переключаемым читом `0/1`.
- Звук сирены на обычной/неполицейской машине игрока по умолчанию выключен и включается отдельным `0/1` читом.
...
В текущей стабильной версии есть два связанных чита:
...
```text
[Мои\Сирена на обычной машине игрока]
Option = Выкл:0
Option = Вкл:1
8011F208 000?
```

Этот пункт включает звук сирены на обычной/неполицейской машине игрока. Реализация сделана без изменения `NFS4.EXE`: один DuckStation/GameShark-чит с условными `D0`-кодами временно отключает две проверки в `AudioClc_SoundPlayersCar`:
...
```text
0  выкл, штатный звук сирены только у полиции
1  вкл, звук сирены работает и на обычной машине игрока
```
...
- Самая важная стабильная точка по сирене на обычной машине игрока: `30b7145`.

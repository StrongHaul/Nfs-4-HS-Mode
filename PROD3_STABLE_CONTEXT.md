...
git       ca7c93b Emit conditional visual damage cheat toggle
```

Проверено пользователем после добавления code-immediate gate для моргания обычных машин и условного `0/1`-переключателя visual-only damage: зависаний не наблюдается, фары/поворотники/белые задние огни работают, полицейские спичи через громкоговоритель работают, damage-only-visual чит работает.

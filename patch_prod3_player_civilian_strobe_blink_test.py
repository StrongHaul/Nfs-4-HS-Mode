...
BACKUP_SUFFIX = ".orig_before_prod3_player_civilian_strobe_blink_test"
...
def cave() -> bytes:
    items: list[int | str | tuple[str, str, str, str]] = [
        ...
        bne("s5", "t0", "done"),
        nop(),
        # Only civilian cars. Player cop cars keep stock blinking.
        slti("t0", "s7", 0x16),
        beq("t0", "zero", "done"),
        nop(),
        # Start palCopy animation if it is not already active.
        lhu("t0", 0x08B8, "s5"),
        andi("t1", "t0", 0x80),
        bne("t1", "zero", "phase"),
        nop(),
        ...
        sh("t0", 0x08BA, "s5"),
        "phase",
        # Do not leave the stock civilian hazard bits active.
        lbu("t0", 0x0447, "s5"),
        andi("t0", "t0", 0xE7),
        sb("t0", 0x0447, "s5"),
        # Visible test: use palCopy bit 3, so the blink is slow enough to see.
        lhu("t0", 0x08B8, "s5"),
        andi("t1", "t0", 0x08),
        bne("t1", "zero", "lights_on"),
        nop(),
        # Off phase: inline the same masks used by R3DCar_TurnHeadLightOff.
        lhu("t0", 0x08B6, "s5"),
        andi("t0", "t0", 0xFFFD),
        sh("t0", 0x08B6, "s5"),
        lhu("t0", 0x08B4, "s5"),
...
def main() -> int:
    ...
    # Accept the previous latch cave as replaceable, without importing it.
    current_cave = bytes(data[CAVE_OFF : CAVE_OFF + CAVE_LEN])
    known_prefixes = {
        bytes(CAVE_LEN),
        blob,
        bytes.fromhex("50 0e 82 8f 11 80 08 3c"),
    }
    if current_cave not in {bytes(CAVE_LEN), blob} and current_cave[:8] not in known_prefixes:
    ...
    print("player civilian strobe blink test:", "reverted" if args.revert else "patched")
    return 0

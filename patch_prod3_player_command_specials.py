...
COMMAND_RESET_CAVE_OFF = 0x45530
...
def make_siren_type_cave() -> bytes:
    items: list[int | str | tuple[str, str, str, str]] = [
        lui("t0", (SPECIALS_ENABLE_ADDR >> 16) & 0xFFFF),
        lhu("t1", SPECIALS_ENABLE_ADDR & 0xFFFF, "t0"),
        andi("t1", "t1", 1),
        beq("t1", "zero", "force_off"),
        nop(),
...
def make_command_reset_cave() -> bytes:
    items: list[int | str | tuple[str, str, str, str]] = [
        lbu("v0", 0x44A, "s0"),
        lui("t0", (SPECIALS_ENABLE_ADDR >> 16) & 0xFFFF),
        lhu("t1", SPECIALS_ENABLE_ADDR & 0xFFFF, "t0"),
    ...
    return pack_labeled(items, runtime(COMMAND_RESET_CAVE_OFF))
...
def patch(exe: Path, *, revert: bool = False) -> None:
    ...
    command_reset_patch = hook_bytes(COMMAND_RESET_CAVE_OFF)
    if dispatch_hook not in (EXPECTED_COMMAND_DISPATCH_HOOK, command_reset_patch):
    ...
    command_reset_cave = make_command_reset_cave()
    ...
    cave_end = max(
        ...
        COMMAND_RESET_CAVE_OFF + len(command_reset_cave),
    ...
    reset_rel = COMMAND_RESET_CAVE_OFF - cave_start
    expected_installed[reset_rel : reset_rel + len(command_reset_cave)] = command_reset_cave
    ...
    if revert:
        ...
        data[PLAYER_COMMAND_CASE11_GATE_OFF : PLAYER_COMMAND_CASE11_GATE_OFF + 4] = EXPECTED_CASE11_GATE
        data[PLAYER_COMMAND_DISPATCH_HOOK_OFF : PLAYER_COMMAND_DISPATCH_HOOK_OFF + 8] = command_reset_patch
        ...
        data[COMMAND_RESET_CAVE_OFF : COMMAND_RESET_CAVE_OFF + len(command_reset_cave)] = command_reset_cave
